...
class Cluster(nn.Module):
    ...
    def contrastive_ema_with_codebook_bank(self, feat, proj_feat, proj_feat_ema, temp=0.07):
        """
        ...
        for batch_ind in range(proj_feat.shape[0]):
            ...
            pos_mask = (codebook_distance > self.thresh_pos)
            neg_mask = (codebook_distance < self.thresh_neg)
            ...
            if self.flat_norm_bank_proj_feat_ema.shape[0] != 0:
                ...
                bank_pos_mask = (bank_codebook_distance > self.thresh_pos)
                bank_neg_mask = (bank_codebook_distance < self.thresh_neg)
...
def compute_modularity_based_codebook(c, x, k, temp=0.1, grid=False):

    # detach
    x = x.detach()

    # pooling for reducing GPU memory allocation
    if grid: x, _ = stochastic_sampling(x, k=k)
